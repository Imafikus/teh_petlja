...
def main():
    ...
    is_first = is_first_player(room_name)


    if is_first:
        wait_for_other_player(room_name)
        my_box, other_box = 'first', 'second'
    else:
        my_box, other_box = 'second', 'first'
    ...
    while not is_game_done(board):
        ...
        if my_turn:
            move = get_my_move()
            send_move(room_name, my_box, move)
        else:
            move = get_other_player_move(room_name, other_box)

        perform_move(move, board, my_turn)
        my_turn = not my_turn
